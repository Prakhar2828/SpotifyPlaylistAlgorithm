...
public class PlaylistReader
{
        ...
        FileNotFoundException
    {
            ...
            && playlistCounter < PlaylistCalculator.NUM_PLAYLISTS)
        {
            ...
            if (tokens.length < 8)
            {
                throw new ParseException(
                    "Insufficient values on the line: " + read,
                    0);
            }
            else if (tokens.length > 8)
            {
                throw new ParseException(
                    "Excess values on the line: " + read,
                    0);
            }
            else
            {
                    ...
                    || !isInValidPercentRange(maxPop, maxRock, maxCountry))
                {
                    throw new DailyMixDataException(
                        "Invalid percentage range in line: " + read);
                }
        ...
        {
            throw new DailyMixDataException(
                "Less than 3 playlists found in the file.");
        }
        ...
        FileNotFoundException
    {
        ...
        while (file.hasNextLine())
        {
                ...
                && tokens.length != SONG_TOKENS - 1)
            {
                ...
                throw new ParseException(
                    "Invalid number of tokens on the line: " + read,
                    0);
            }
            ...
            {
                throw new ParseException(
                    "Invalid number format in line: " + read,
                    0);
            }
            ...
            {
                throw new DailyMixDataException(
                    "Invalid genre set values on the line: " + read);
            }
